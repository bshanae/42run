...
class							game::bonus::coin : public game::bonus::bonus
{
	friend class				game::manager;

public :

	explicit					coin(line line) : bonus(line, state::running, global_model())
	{
...
private :
	...
	shared<model::model>		global_model()
	{
		...
		if (not model)
			model = make_shared<model::model>(sources().bonus_coin, model::flag::center);
		return (model);
	}
};